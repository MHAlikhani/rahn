...
v0.1 execution is simulation-only (ADR 0008). rahn apply MUST produce an explicit, recorded execution plan and MUST NOT modify the host system. The executor MUST accept only transitions carrying a passing verification result. Real backends MUST implement a plan-execution interface and MUST NOT leak backend concepts into the state model. The namespace backend (ADR 0012) MUST map plans to iproute2 argv deterministically, MUST restrict host-side operations to `ip netns add/del rahn-*`, MUST refuse execution on non-Linux platforms, and MUST refuse execution without an explicit opt-in flag pair (`--execute --yes-i-know`). The default `apply` MUST print the command sequence and perform no I/O.
