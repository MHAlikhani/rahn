...
Rust workspace with small crates: `rahn-core`, `rahn-state`, `rahn-store`, `rahn-verify`, `rahn-cli`, `rahn-sim`, `rahn-exec`, `rahn-dist`, `rahn-sdk`. Persistence: filesystem + canonical serialized objects (format v2). The topology model is interface-based (ADR 0011): links connect `(node, interface)` endpoints. Deterministic graph queries (shortest path with lexicographic tie-break, reachability, components) and isolation constraints (`prohibit-connectivity`) are implemented. Execution: simulation-first — `apply` prints the exact iproute2 commands; real execution is opt-in (`--execute --yes-i-know`) against isolated Linux network namespaces + veth (ADR 0012), with host-side operations restricted to `rahn-*` namespaces and CI validation. No addressing yet, so connectivity is link-existence only. Observability (Stage 4): deterministic `Observation` records (ADR 0013) in an append-only log, associated with states by validated provenance. Causal memory (Stage 5): explicit status-labeled edges between observations and commits (ADR 0014), DAG-enforced, never inferred. Distributed state (Stage 6): peer sync over content-addressed history (`rahn-dist`, ADR 0015) — per-replica authority, deterministic byte-identical convergence, fail-closed divergence; no leader election, no cross-replica strong-consistency claims. Execution backends (Stage 7): `ExecutionBackend` trait (ADR 0016) with capability negotiation — `simulation` (default, description-only) and `linux-ns` (ADR 0012 semantics) behind the same interface; dataplane backends are future trait implementations. (See [docs/adr/0004-storage-model.md](docs/adr/0004-storage-model.md), [docs/adr/0008-execution-boundary.md](docs/adr/0008-execution-boundary.md).)
...
- **Simulation-first over execution-first.** Simulation is the default; real execution is opt-in against isolated Linux network namespaces (ADR 0012); accepted to protect the state model from execution concerns.
